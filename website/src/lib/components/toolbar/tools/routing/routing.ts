...
import { PUBLIC_BROUTER_URL } from '$env/static/public';
...
async function getRoute(
    ...
    let url = `${PUBLIC_BROUTER_URL}?lonlats=${points.map((point) => `${point.lon.toFixed(8)},${point.lat.toFixed(8)}`).join('|')}&profile=${brouterProfile + (privateRoads ? '-private' : '')}&format=geojson&alternativeidx=0`;
